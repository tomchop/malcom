...
import Malcom.intel.web.views as intel
app.register_blueprint(malcom_websockets, url_prefix='/websocket')
app.register_blueprint(malcom_api, url_prefix='/api')
app.register_blueprint(intel.malcom_intel, url_prefix=intel.PREFIX, static_folder='static')
...
def find_related(field, query, base_elts, base_ids, evil_elts):
    ...
    nodes = { n['_id']: n for n in data['nodes']}
    edges = { e['_id']: e for e in data['edges']}
    base_elts = { e['_id']: e for e in base_elts}

    outgoing_links, incoming_links = {}, {}

    for e in edges.values():
        if e['src'] in base_elts:
            d = outgoing_links
        if e['dst'] in base_elts:
            d = incoming_links
